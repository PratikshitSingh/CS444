    ...
    def calc_gradient(self, X_train: np.ndarray, y_train: np.ndarray) -> np.ndarray:
        ...
        for x, y in zip(X_train, y_train):
            ...
            for c in range(0, self.n_class):
                exp_wt_x.append(np.exp(np.dot(self.w[c, :], x)/T))
            
            normalization = sum(exp_wt_x)

            '''for correct class y'''
            gradient[y, :] += self.w[y, :] * self.reg_const/batch_size
            gradient[y, :] -= x
            gradient[y, :] += (exp_wt_x[y]/normalization) * x

            '''for incorrect class y!=c'''
            for c in range(0, self.n_class):
                if c!=y:
                    gradient[c, :] += self.w[c, :] * self.reg_const/batch_size
                    gradient[c, :] += (exp_wt_x[c]/normalization) * x
