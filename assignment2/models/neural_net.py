...
class NeuralNetwork:
    ...
    def __init__(
        ...
        sizes = [input_size] + hidden_sizes + [output_size] #list of number of units in each layer
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        # Apply the relu activation funtion only uptil last-1 layer
        for i in range(1, self.num_layers):
            W = self.params["W" + str(i)]
            b = self.params["b" + str(i)]

            self.outputs["output" + str(i)] = self.relu(self.linear(W, input, b))
            input = self.outputs["output" + str(i)]

        # Apply softmax prediction function for the last layer
        output = self.softmax(self.linear(self.params["W" + str(self.num_layers)], input, self.params["b" + str(self.num_layers)]))
        return output
